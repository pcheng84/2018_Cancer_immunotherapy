...
We have a table with some clinical data. It is a tab-delimited table we set `sep = "\t"`. We also know the first line of the table are the names of the column so we set `header = TRUE`.
```{r, message = TRUE, warning = FALSE, echo = TRUE, eval = TRUE}
MyData <- read.table("Derm_patients.txt", sep = "\t", header = TRUE)
```
...
#get the last 4 rows of the table
tail(MyData)
```
